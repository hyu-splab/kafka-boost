...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  def numDedicatedThreads = getInt(SocketServerConfigs.NUM_DEDICATED_THREADS_CONFIG)
  def boosterCheckIntervalMs = getInt(SocketServerConfigs.BOOSTER_CHECK_INTERVAL_MS_CONFIG)
  def boosterNewChannelCooldownTicks = getInt(SocketServerConfigs.BOOSTER_NEW_CHANNEL_COOLDOWN_TICKS_CONFIG)
  def boosterGlobalReassignCooldownTicks = getInt(SocketServerConfigs.BOOSTER_GLOBAL_REASSIGN_COOLDOWN_TICKS_CONFIG)
  def boosterChannelReassignCooldownTicks = getInt(SocketServerConfigs.BOOSTER_CHANNEL_REASSIGN_COOLDOWN_TICKS_CONFIG)
  def boosterSaturationWindowSize = getInt(SocketServerConfigs.BOOSTER_SATURATION_WINDOW_SIZE_CONFIG)
  def boosterIdleWindowSize = getInt(SocketServerConfigs.BOOSTER_IDLE_WINDOW_SIZE_CONFIG)
  def boosterRequestsStatWindowSize = getInt(SocketServerConfigs.BOOSTER_REQUESTS_STAT_WINDOW_SIZE_CONFIG)
