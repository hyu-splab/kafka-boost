...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  def boosterAssignCooldownTicks = getInt(SocketServerConfigs.BOOSTER_ASSIGN_COOLDOWN_TICKS_CONFIG)
  def boosterReturnCooldownTicks = getInt(SocketServerConfigs.BOOSTER_RETURN_COOLDOWN_TICKS_CONFIG)
